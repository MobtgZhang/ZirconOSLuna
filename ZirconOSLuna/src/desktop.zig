//! Desktop Manager - ZirconOS Luna Desktop
//! Manages the desktop surface including wallpaper rendering,
//! icon layout, right-click context menu, and desktop properties.
...
pub const DesktopSettings = struct {
    show_my_computer: bool = true,
    show_my_documents: bool = true,
    show_network_places: bool = true,
    show_recycle_bin: bool = true,
    show_internet_explorer: bool = true,
    auto_arrange: bool = true,
    align_to_grid: bool = true,
    arrange_by: ArrangeBy = .name,
...
pub fn showContextMenu(x: i32, y: i32) void {
    ...
    addContextItem("Arrange Icons By", .submenu, 100);
    addContextItem("Refresh", .normal, 101);
    addContextItem("", .separator, 0);
    addContextItem("Paste", .disabled, 102);
    addContextItem("Paste Shortcut", .disabled, 103);
    addContextItem("", .separator, 0);
    addContextItem("New", .submenu, 110);
    addContextItem("", .separator, 0);
    addContextItem("Properties", .normal, 120);
}
...
fn createSystemIcons() void {
    if (settings.show_my_computer) {
        _ = addIcon("My Computer", "C:\\", .my_computer, true);
    }
    if (settings.show_my_documents) {
        _ = addIcon("My Documents", "C:\\Documents and Settings\\User\\My Documents", .my_documents, true);
    }
    if (settings.show_network_places) {
        _ = addIcon("My Network Places", "\\\\Network", .network_places, true);
    }
    if (settings.show_recycle_bin) {
        _ = addIcon("Recycle Bin", "C:\\RECYCLER", .recycle_bin, true);
    }
    if (settings.show_internet_explorer) {
        _ = addIcon("Internet Explorer", "C:\\Program Files\\Internet Explorer\\iexplore.exe", .internet_explorer, true);
    }
}
